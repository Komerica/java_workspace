...
//   이때, 추상클래스(abstract class)를 만들어서 상위 클래스(추상 클래스)에서는 공통적으로 사용되는 로직을 구현하고,
//   그것을 상속 받은 하위클래스에서는 용도에 따라서 달라지는 구현을 사용자가 직접하도록 유도하기 위해서 abstract을 사용한다!
...
// 그 logic을 사용하기 위한 형식이라고 할 수 있는 signature 만을 갖고 있다.
// -> 여기서 말하는 signature 는 ↓
//    abstract method 일 경우에는, public abstract int b();
//    abstract class 일 경우에는, abstract class A_abstract{};
// 즉, 그 signature에 해당되는 method에 대한 구체적인 구현(logic)의 책임은 그것을 사용하는 쪽에게 넘기는 것을 abstract라고 함!
